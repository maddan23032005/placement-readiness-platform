...
export async function GET(req: NextRequest) {
  ...
  const questions = await db.questionMCQ.findMany({
    where: {
...
export async function POST(req: NextRequest) {
  ...
  const { text, options, correctOptions, topic, difficulty, tags } = parsed.data;

  const question = await db.questionMCQ.create({
    data: {
      text,
      options,
      correctOptions,
